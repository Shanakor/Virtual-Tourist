...
class APIClient {
    ...
    public func taskForData(with url: URL, completionHandler: @escaping (Data?, APIClientError?) -> Void = { _, _ in }){
        ...
        let task = URLSession.shared.dataTask(with: url) {
            ...
            // GUARD: Did the request fail?
            guard error == nil else {
                completionHandler(nil, .connectionError(description: "Request at url '\(url)' failed."))
                return
            }
